...
final class DateViewCell: UICollectionViewCell {
    ...
    var date: Date = Date() {
        didSet {
            ...
            dotViewHeightConstraint.constant = emptyDot ? 0 : 0//format.dotWidth
    ...
    var format = DateScrollPickerFormat() {
        didSet {
            ...
            containerView.layer.cornerRadius = format.dayRadius + 20
    ...
    var isOn: Bool = false {
        didSet {
            ...
            if isOn{
                ...
                self.addShadow(color: UIColor.App_SemiBlue ?? UIColor(), opacity: 0.5, offSet: .init(width: 0, height: 0), radius: 10)
            }
            else {
                self.addShadow(color: .gray, opacity: 0, offSet: .init(width: 0, height: 2), radius: 0)
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        dotView.layer.cornerRadius = 2.5//5
